...
import { ver2QueryResults } from 'src/assets/ver2-queryResults';
import { ver1_queryResults } from 'src/assets/ver1-queryResults';
import { HAMLET } from 'src/assets/hamlet';
import { HAMLE_VERSION2 } from 'src/assets/hamelet_version2';
import { HAMLE_VERSION3 } from 'src/assets/hamlet_version3';
import { hamletQueryResults } from 'src/assets/hamlet-queryResults';
import { hamletQueryResults2 } from 'src/assets/hamlet_version2_queryResults';
...
import { englishHemlet } from 'src/assets/English_hemlet';
import { englishQueries } from 'src/assets/english_queries';
@Component({
  selector: 'app-root',
...
export class AppComponent {
  activeView: 'ai' | 'shakespeare' | 'hamlet' | 'german' = 'ai';
  
  // Data for dual-view-content component
  shakespeareChartData1 = Version1;
  shakespeareChartData2 = Version2;
  hamletChartData1 = HAMLET;
  hamletChartData2 = HAMLE_VERSION2;
  hamletChartData3 = HAMLE_VERSION3;
  shakespeareQueryResults1 = ver1_queryResults;
  shakespeareQueryResults2 = ver2QueryResults;
  hamletQueryResults1 = hamletQueryResults;
  hamletQueryResults2 = hamletQueryResults2;
  hamletQueryResults3 = hamletQueryResults3;
  englishChartData = englishHemlet;
  englishQueryResults = englishQueries;
  ...
  germanQuestions: string[] = [
    'Welche Ereignisse vor dem Beginn von Hamlet bereiten den zentralen Konflikt vor?',
    'Warum ist das Erscheinen des Geistes in Akt 1 von Bedeutung?',
    'Wie rechtfertigt Claudius seine schnelle Heirat mit Gertrude nach dem Tod von König Hamlet?',
    'Wie führt das Stück die Spannungen zwischen Norwegen und Dänemark ein?',
    'Warum trauert Hamlet so tief, während der Hof scheinbar bereit ist weiterzumachen?',
    'Wie beschreibt Hamlet die Wiederheirat seiner Mutter und warum verstört ihn diese so sehr?',
    'Was erfährt Hamlet vom Geist, und wie beeinflusst dies seine Mission?',
    'Wie behandelt Hamlet Horatio, als sie sich in Akt 1 wiedersehen?',
    'Wie stellt Claudius in seiner Eröffnungsrede seinen Aufstieg zum Thron dar?',
    'Welches politische Problem im Zusammenhang mit Fortinbras spricht Claudius an?'
  ];
//   hamletQuestions: string[] = []; // Add questions if needed
  prompt: string = '';
  response: any;
  umapData: any = {
    "quantum Computing": {
...
rootWordCloud: any = [
  ...
  hamletQuestions: string[] = [
    "What events happen before the beginning of Hamlet that set up the central conflict?",
    "How does Claudius justify marrying Gertrude so soon after King Hamlet's death?",
    "Why is Hamlet grieving so deeply while the court seems ready to move on?",
    "How does Hamlet describe his mother's remarriage and why does it disturb him?",
    "What does Hamlet learn from the ghost, and how does this shape his mission?",
    "How does Hamlet treat Horatio when they reunite in Act 1?",
    "How does Claudius frame his rise to the throne in his opening speech?",
    "What political problem involving Fortinbras does Claudius address?"
  ];
 selectedButton: any = 'tsvg' 
umapData2: any = Data2;
umapData3: any = Data3;
  load:boolean = false
...
let obj2:any = []
let obj3:any = []
Object.keys(this.umapData3).forEach((data: any) => {
    obj1.push(data)
})
Object.keys(this.umapData2).forEach((data: any) => {
    obj2.push(data)
})
Object.keys(this.umapData).forEach((data: any) => {
    obj3.push(data)
})
  ...
  sendMessage() {
    ...
    this.openaiService.sendQuery(this.userMessage,this.field2,this.field1).subscribe((response: any) => {
      ...
      this.extractedText = response.results.matrix;
    });
  ...
  let select: any = {}
    if(data.subtopic) {
        ...
        select.level2 = true
        select.level1 = true
        select.selectedSubTopic = data.key
        select.selectedTopic = data.subtopic
        select.selectedRoot = data.topic
    } else if(data.topic){
        select.level3 = false
        select.level2 = true
        select.level1 = true
        select.selectedTopic = data.key
        select.selectedSubTopic = ''
        select.selectedRoot = data.topic
    } else {
        select.level3 = false
        select.level2 = false
        select.level1 = true
        select.selectedRoot = data.key
    }
  ...
  handleHeaderButtonClick(buttonName: string) {
    console.log('Selected category:', buttonName);
  ...
  switchView(): void {
    if (this.activeView === 'ai') {
      this.activeView = 'shakespeare';
    } else if (this.activeView === 'shakespeare') {
      this.activeView = 'hamlet';
    } else if (this.activeView === 'hamlet') {
      this.activeView = 'german';
    } else {
      this.activeView = 'ai';
    }
  }
